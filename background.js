      ...
      async function processAuthToken(token, tabId = null) {
        try {
          if (token && token !== lastCapturedToken) {
            ...
            if (tabId) setTimeout(() => chrome.tabs.remove(tabId), 3e3);
          }
        } catch (e) {
